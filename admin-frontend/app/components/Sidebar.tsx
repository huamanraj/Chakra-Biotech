...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: 'Blog Categories', href: '/dashboard/blog-categories', icon: FolderOpen },
    { name: 'Products', href: '/dashboard/products', icon: Package },
    { name: 'Product Categories', href: '/dashboard/product-categories', icon: Grid3X3 },
      ...
      <AnimatePresence>
        ...
        >
          <div className="flex flex-col h-full">
            {/* Logo */}
            <div className={`border-b border-admin-200 ${isDesktopCollapsed && !isMobile ? 'p-4' : 'p-6'}`}>
                ...
                <AnimatePresence>
                  {(!isDesktopCollapsed || isMobile) && (
                    <motion.div
                      ...
                      transition={{ duration: 0.2 }}
                    >
                      <h1 className="font-serif text-xl font-bold text-admin-800 whitespace-nowrap">Saffron</h1>
                      <p className="text-sm text-admin-500 whitespace-nowrap">Admin Dashboard</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
                ...
                return (
                  ...
                  >
                    <Link href={item.href} onClick={closeMobileMenu}>
                      <div className={`group relative flex items-center space-x-3 px-3 py-3 rounded-xl transition-all duration-300 hover:bg-admin-50 cursor-pointer ${
                        isActive 
                        ...
                        <item.icon className={`w-5 h-5 flex-shrink-0 transition-colors ${
                          isActive ? 'text-saffron-600' : 'text-admin-500 group-hover:text-admin-700'
                        }`} />
                        <AnimatePresence>
                          {(!isDesktopCollapsed || isMobile) && (
                            <motion.span
